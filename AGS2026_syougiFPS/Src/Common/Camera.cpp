...
void Camera::UpdateFPS(void)
{
    ...
    // ★ 視点移動を 右スティック (RX, RY) に変更
    float padRX = static_cast<float>(padState.AKeyRX);
    float padRY = static_cast<float>(padState.AKeyRY);

    // 入力値の正規化 (-1.0f ～ 1.0f)
    if (fabsf(padRX) > 1000.0f) padRX /= 32767.0f;
    else                        padRX /= 1000.0f;

    if (fabsf(padRY) > 1000.0f) padRY /= 32767.0f;
    else                        padRY /= 1000.0f;

    // 範囲制限 [-1.0f, 1.0f]
    if (padRX > 1.0f)  padRX = 1.0f;
    if (padRX < -1.0f) padRX = -1.0f;
    if (padRY > 1.0f)  padRY = 1.0f;
    if (padRY < -1.0f) padRY = -1.0f;

    const float deadZone = 0.2f;       // デッドゾーン
    const float padTurnSpeed = 0.03f;  // 視点回転速度（お好みで調整）

    // ★ 右スティック X (左右) -> deltaX (横回転)
    if (fabsf(padRX) > deadZone) {
        deltaX += padRX * padTurnSpeed;
    }
    // ★ 右スティック Y (上下) -> deltaY (縦回転)
    if (fabsf(padRY) > deadZone) {
        deltaY += padRY * padTurnSpeed; // 上下が逆と感じる場合は -= に変更
    }
